...
export default function DrawTable({dataSource}: {dataSource: TableFields[]}) {
  const countryFilter = unique(dataSource.map(r => r.country)).map(r => ({text: r, value: r}))
  const starFilter = unique(dataSource.map(r => r.stars)).map(r => ({text: <img src={starsToIcon(r)} alt="Michelin stars" className='h-4 inline'/>, value: r}))
  const yearFilter = unique(dataSource.map(r => r.year)).map(r => ({text: r, value: r}))
  ...
  const columns: ColumnsType<TableFields> = [
    ...
    },
    {
      ...
      render: (_value, record) => <img src={starsToIcon(record.stars)} alt="Michelin stars" className='h-5'/>
    },
    {
      title: 'Food',
      dataIndex: 'food',
